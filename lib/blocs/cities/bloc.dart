...
class CitiesBloc extends Bloc<CitiesEvents, CitiesStates> {
  ...
  void _sendData(CitiesEvent event,
      Emitter<CitiesStates> emit,) async {
    list = [];
    model = null;
    emit(CitiesLoadingState());

    final response = await ServerGate.i.getFromServer(url: "general/cities/${event.countryId}");

    if (response.success) {
      list = List.from(response.data).map((e) => CityModel.fromJson(e)).toList();
